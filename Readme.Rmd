...
* `SF4/` (PKS KS domain phylogenetic tree)
...
* `SF9/` (Intron number in HGT and fungal genes)
...
* `SF10/` (5-mer assay in HGT and fungal genes)
...
* `SF11/` (Codon usage assay in HGT and fungal genes)
...
"Secondary_Metabolyte_raw"
zygo.all.m$gene_fam[zygo.all.m$variable %in% c("SM_normalized")] <- "Proportion of SM per total genes"

# Plot
ggplot(zygo.all.m[!zygo.all.m$gene_fam %in% c("Secondary_Metabolyte_raw"),]) + geom_bar(aes(x=Isolate.Species, y=value, fill=variable), stat="identity") + 
...
## Sup. Figure 5: Fungal PKS

```{r, fig.height=12}
dom.df.m <- readRDS("SF5/PKS_Fungi.Rds")
...
## Sup. Figure 6: Zygo PKS

```{r, fig.height=12}
dom.df <- read.table("SF6/PKS_counts.txt", sep = "\t", header = T, comment.char = '')
...
## Sup. Figures 9: Intron Number

```{r, fig.height=12}
intron.df <- readRDS("SF9/intron_number.Rds")
...
## Sup. Figure 10: Pentamers (Local)

```{r, fig.height=12}
pca.penta <- readRDS(file = "SF10/pentameres_simple.Rds")
...
## Sup. Figure 11. Codon Usage

```{r, fig.height=12}
pca.df <- readRDS(file = "SF11/codon_usage_simple.Rds")
